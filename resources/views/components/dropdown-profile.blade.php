...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        ...
        <div class="flex items-center truncate">
            <span class="truncate ml-2 text-sm font-medium text-gray-600 group-hover:text-gray-800">{{ Auth::user()->name }}</span>
    ...
    >
        ...
        <ul>
            <li>
                <a class="font-medium text-sm text-[#98d0c0] hover:text-[#3c483d] flex items-center py-1 px-3" href="{{ route('adminprofile.edit') }}" @click="open = false" @focus="open = true" @focusout="open = false">Hồ sơ cá nhân</a>
            </li>
            <li>
                <form method="POST" action="{{ route('logout') }}" x-data>
                    @csrf

                    <a class="font-medium text-sm text-[#98d0c0] hover:text-[#3c483d] flex items-center py-1 px-3"
                        href="{{ route('logout') }}"
